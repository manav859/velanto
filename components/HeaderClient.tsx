...
// Defined here (not imported from lib/shopify) to avoid pulling server-only
// cacheLife/cacheTag APIs into a client component bundle.
...
// ── Static nav segments ────────────────────────────────────────────────────────
// These never depend on Shopify data and always render.

const BEFORE_COLLECTIONS = [
  { label: 'Home', href: '/' },
  { label: 'Shop', href: '/shop' },
]

const AFTER_COLLECTIONS = [
...
type Props = {
  /** Dynamic collection links fetched server-side. Empty = no collection nav items shown. */
  navCollections: NavCollectionItem[]
}

export default function HeaderClient({ navCollections }: Props) {
  const [menuOpen, setMenuOpen] = useState(false)
  const { openCart, totalItems } = useCart()

  // Merge static + dynamic nav into a single ordered list
  const allLinks = [
    ...BEFORE_COLLECTIONS,
    ...navCollections.map((c) => ({ label: c.label, href: c.href })),
    ...AFTER_COLLECTIONS,
  ]
    ...
    <header className="sticky top-0 z-40 w-full border-b border-white/8 bg-background/90 backdrop-blur-md">
          ...
          <nav className="hidden lg:flex items-center gap-1 overflow-x-auto">
            {allLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="shrink-0 px-3 py-2 text-sm font-medium text-zinc-400 hover:text-white transition-colors duration-150 rounded-md hover:bg-white/5"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <AnimatePresence mode="wait" initial={false}>
                {menuOpen ? (
                  <motion.svg key="close" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }} transition={{ duration: 0.15 }} className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                  </motion.svg>
                ) : (
                  <motion.svg key="open" initial={{ rotate: 90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -90, opacity: 0 }} transition={{ duration: 0.15 }} className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            <nav id="mobile-navigation" className="flex flex-col px-4 py-3 gap-0.5">
              {allLinks.map((link, i) => (
                <motion.div
                  key={link.href}
